        ...
        describe('edit', () => {
            describe('with existing user', () => {
                ...
                //     )
                    ...
                    it('should succeed', () => {
                        let newText = `${text} 123`
                        logic.updatePostit(postitId, userId, _token, newText)
                            .then(() => expect(true).to.be.true)
                            .then(() => logic.listPostitsByUser(userId, _token))
                            .then(()=>{
                                // postits => let mypost = postits.filter(post => post.userId === userId)
                                expect(mypost.text).to.equal(newText)
                                } 
                    })
                // })
            })
